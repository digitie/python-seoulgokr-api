...
- backend egress 또는 HTTPS proxy를 운영자가 신뢰하는 경우에만
  ...
  않는다.
- 공개되지 않은 서비스별 일 quota는 코드에서 추정하지 않는다. 기본 daily budget은
  무제한이며, 소비자가 `service_daily_budgets`로 자체 예산을 설정한다.
- 실시간 지하철의 보수적 기본 간격은 30초로 둔다. 전체역 도착은 기본 비활성이고,
  명시적 opt-in과 `all_station_arrivals_max_items` 상한이 모두 필요하다.
- 429의 `Retry-After`는 동일 key scope의 process 내 limiter에 공유 cooldown으로
